	...
	private class Eventos implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent evento) {
			if (evento.getSource() == botaoVoltar) {
				...
				} else if (cbMesInicial.getSelectionModel().getSelectedIndex() > cbMesFinal
						.getSelectionModel().getSelectedIndex()) {
					labelAviso
							.setText("Selecione um mês final que seja igual ou posterior ao mês inicial.");
					labelAviso.setVisible(true);
				} else {

					try {
						relatorio = new Relatorio(usuarioAtivo.getContaAtiva());
						relatorio.filtraPorCategoria(cbCategoria
								.getSelectionModel().getSelectedItem());
						relatorio.filtraPorData(cbMesInicial
								.getSelectionModel().getSelectedIndex() + 1,
								cbMesFinal.getSelectionModel()
										.getSelectedIndex() + 1);
						relatorio.filtraPorTipo((String) grupoTipoTransacao
								.getSelectedToggle().getUserData());

						String tipoRelatorio = (String) grupoTipoRelatorio
								.getSelectedToggle().getUserData();
						...
						if (tipoRelatorio.equals("histograma")) {
							...
							for (int i = 0; i < 12; i++) {
								if (i >= cbMesInicial.getSelectionModel()
										.getSelectedIndex()
										&& i <= cbMesFinal.getSelectionModel()
												.getSelectedIndex()) {
							...
							try {
								FXMLLoader fxmlLoader = new FXMLLoader(
										getClass().getResource(
												"../gui/TelaHistograma.fxml"));
								Parent root = (Parent) fxmlLoader.load();
								ControllerHistograma controller = fxmlLoader
										.<ControllerHistograma> getController();
								controller.inicializa(usuarioAtivo, relatorio,
										cbCategoria.getSelectionModel()
												.getSelectedItem(),
										(String) grupoTipoTransacao
												.getSelectedToggle()
												.getUserData(), meses,
										cbMesInicial.getSelectionModel()
												.getSelectedIndex(), cbMesFinal
												.getSelectionModel()
												.getSelectedIndex());
								content.getChildren().setAll(root);
							} catch (IOException e) {
							...
							try {
								FXMLLoader fxmlLoader = new FXMLLoader(
										getClass().getResource(
												"../gui/TelaLista.fxml"));
								Parent root = (Parent) fxmlLoader.load();
								ControllerLista controller = fxmlLoader
										.<ControllerLista> getController();
								controller.inicializa(usuarioAtivo, relatorio);
								content.getChildren().setAll(root);
							} catch (IOException e) {
								e.printStackTrace();
