...
public class ControllerAdicionarTransacao {
	...
	private final ObservableList<String> recorrencias = FXCollections
			.observableArrayList("Nenhuma", "Semanal", "Mensal");
	...
	private class Eventos implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent evento) {
			if (evento.getSource() == botaoVoltar) {
				...
				try {
					passouOrcamento = usuarioAtivo.getContaAtiva()
							.adicionaTransacao(
									tfDescricao.getText(),
									data.getValue(),
									tfValor.getText(),
									cbCategoria.getSelectionModel()
											.getSelectedItem(),
									cbRecorrencia.getSelectionModel()
											.getSelectedItem(),
									(String) group.getSelectedToggle()
											.getUserData());
					...
					if (passouOrcamento) {
						Dialogs.create()
								...
								.masthead(null)
								.message(
										"O valor do orçamento de despesa desta categoria foi ultrapassado.")
								.showInformation();
					}
					...
					Dialog.Actions resposta = (Actions) Dialogs
							...
							.masthead(null)
							.message(
									"Transação efetuada. Deseja adicionar uma nova transação?")
							.showConfirm();
					...
					if (resposta == Dialog.Actions.YES) {
						try {
							FXMLLoader fxmlLoader = new FXMLLoader(
									getClass()
											.getResource(
													"../gui/TelaAdicionarTransacao.fxml"));
							Parent root = (Parent) fxmlLoader.load();
